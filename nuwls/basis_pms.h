#ifndef _NUWLS_BASIS_PMS_H_
#define _NUWLS_BASIS_PMS_H_
...
inline constexpr float MY_RAND_MAX_FLOAT = 10000000.0;
inline constexpr int MY_RAND_MAX_INT = 10000000;
inline constexpr float BASIC_SCALE = 1.0f / MY_RAND_MAX_FLOAT;
...
class ISDist
{
  ...
  public:
	ISDist() = default;
	void settings(int _max_tries = 10, int _max_non_improve_flip = 10000);
	void build_instance(char *filename);
	void local_search_with_decimation(int mx_cpu_second, std::vector<int> sol_init = {});
	void simple_print();
	...
	void build_instance(int nvar, int mx_weight, const std::vector<std::vector<int>> &hard, const std::vector<std::vector<int>> &soft, const std::vector<int> weight);
	void get_best_solution(std::vector<int> &sol);
};
