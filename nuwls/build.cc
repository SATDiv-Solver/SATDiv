...
void ISDist::settings(int _max_tries, int _max_non_improve_flip)
{
    best_known = -2;
    ...
    max_non_improve_flip = _max_non_improve_flip;
...
void ISDist::build_instance(int nvar, int mx_weight, const vector<vector<int>> &hard, const vector<vector<int>> &soft, const vector<int> weight)
{
    total_soft_length = 0;
    total_hard_length = 0;

    num_vars = nvar;
    num_clauses = hard.size() + soft.size();
    top_clause_weight = mx_weight;
    ...
    // TODO. replace read file
    // 去掉一定为全是 True 的字句
    num_hclauses = hard.size();
    num_sclauses = soft.size();
    if (num_sclauses > 0)
        problem_weighted = 1;
    
    for (const auto &cls : hard) {

        org_clause_weight[c] = mx_weight;
        
        clause_lit_count[c] = cls.size();
        clause_lit[c] = new lit[clause_lit_count[c] + 1];

        int i;
        for (i = 0; i < clause_lit_count[c]; ++i) {
            clause_lit[c][i].clause_num = c;
            clause_lit[c][i].var_num = abs(cls[i]);
            if (cls[i] > 0)
    ...
    int si = 0;
    for (const auto &cls : soft) {
        
        org_clause_weight[c] = weight[si];
        total_soft_weight += weight[si];
