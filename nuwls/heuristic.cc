...
static double get_runtime()
{
    struct tms ctime;
    times(&ctime);
    return (double)(ctime.tms_utime + ctime.tms_stime) / sysconf(_SC_CLK_TCK);
}
...
void ISDist::local_search_with_decimation(int mx_cpu_second, std::vector<int> sol_init)
{
    ...
    for (tries = 1; tries < max_tries && (mx_cpu_second == -1 || opt_time < mx_cpu_second); ++tries)
    {   
        // cout << "tries = " << tries << '\n';
        if (tries == 1 && !sol_init.empty()) {
            std::vector<int> internal_sol(num_vars + 1);
            if (sol_init.size() == num_vars) {
                for (int i = 0; i < num_vars; ++i) {
                    internal_sol[i + 1] = sol_init[i];
                }
            } else {
                internal_sol = sol_init; 
            }
        ...
        for (step = 1; step < max_flips && (mx_cpu_second == -1 || opt_time < mx_cpu_second); ++step)
        {
            last_times --;
            if (last_times <= 0) {
                cur_time = get_runtime() - start_time;
                last_times = 1000;

                if (mx_cpu_second > 0 && cur_time > mx_cpu_second)
                    return ;
            }
            
            // cout << "opt_time = " << opt_time << ", step = " << step << '\n';
            if (hard_unsat_nb == 0)
            {
                local_soln_feasible = 1;
                ...
                {   
                    max_flips = step + max_non_improve_flip;
                    opt_time = get_runtime() - start_time;
                    ...
                    if (mx_cpu_second > 0 && opt_time > mx_cpu_second)
                    {
                        best_soln_feasible = 1;
                        cout << "c optimize timeout" << endl;
                        if (opt_unsat_weight < best_known)
                        {
                            cout << "c a better solution " << opt_unsat_weight << endl;
                        }
